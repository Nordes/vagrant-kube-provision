...
sudo kubeadm init --control-plane-endpoint kube-lb --apiserver-advertise-address $1 --upload-certs > $2/kubeadm-install.log
...
sleep 10
# Calico creates issues, so better use weave
...
grep "kubeadm join" $2/kubeadm-install.log -A 2 | tail -n2 > $2/kubeadm-node.sh
sudo sed -i '1 i\#/bin/bash' $2/kubeadm-node.sh

sudo chmod +x $2/kubeadm-*.sh
